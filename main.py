...
# Emojis (opzionali, potrebbero non essere visualizzati correttamente su tutti i terminali)
EMOJI_SUCCESS = "✅"
EMOJI_FAIL = "❌"
EMOJI_WARN = "⚠️"
EMOJI_INFO = "ℹ️"
...
def verify_signature(data: bytes, signature_hex: str, public_key_pem: bytes) -> bool:
    """
    ...
    try:
        ...
        print(f"{Colors.FAIL}{EMOJI_FAIL} Errore durante la verifica della firma: {e}{Colors.ENDC}")
        return False
...
def insert_signature_chain(document: bytes, signer: str, conn, private_key_pem: bytes, is_first_call: bool, 
    ...
    """
    if is_first_call:
        print(f"\n{Colors.HEADER}{EMOJI_CHAIN}==== Sequenza Firme Inserite nella Catena ===={Colors.ENDC}")
        print(f"{Colors.OKCYAN}Documento Originale: \"{original_doc_content}\"{Colors.ENDC}")
        print(f"{Colors.OKCYAN}Hash Documento Originale: {hash_document(document)}{Colors.ENDC}")
        print(Colors.OKCYAN + "-" * 70 + Colors.ENDC)

    document_hash = hash_document(document)
    cursor = conn.cursor()

    cursor.execute("SELECT signature FROM signature_chain ORDER BY id DESC LIMIT 1")
    row = cursor.fetchone()
...
def verify_chain(conn, firmatari_data: dict, user_context: str = ""):
    ...
    if not records:
        print(f"{Colors.WARNING}{EMOJI_INFO} Nessuna firma trovata nella catena per la verifica.{Colors.ENDC}")
        return True 
    ...
    for i, row_data in enumerate(records):
        ...
        if i == 0: 
            if prev_hash_stored is not None:
                print(f"  {Colors.FAIL}{EMOJI_FAIL} ERRORE: Il blocco genesi (ID: {record_id}) dovrebbe avere prev_hash NULL, invece è '{prev_hash_stored}'.{Colors.ENDC}")
                is_chain_valid = False
        else: 
            if prev_hash_stored != last_block_signature:
                print(f"  {Colors.FAIL}{EMOJI_FAIL} ERRORE: prev_hash del blocco {record_id} ('{prev_hash_stored}') non corrisponde alla signature del blocco precedente ('{last_block_signature[:10]}...').{Colors.ENDC}")
                is_chain_valid = False
            else:
        ...
        chain_input_to_verify = (prev_hash_stored or '').encode() + doc_hash_stored.encode()
        
        if verify_signature(chain_input_to_verify, current_signature_stored, public_key_pem):
            print(f"  {Colors.OKGREEN}{EMOJI_SUCCESS} OK: La firma del blocco {record_id} è valida.{Colors.ENDC}")
        else:
            print(f"  {Colors.FAIL}{EMOJI_FAIL} ERRORE: La firma del blocco {record_id} NON è valida (possibile manomissione di document_hash o prev_hash).{Colors.ENDC}")
            is_chain_valid = False
...
def clear_signature_table(db_name_param, super_user_param, super_password_param, db_host_param):
    ...
    try:
        ...
        print(f"{Colors.OKGREEN}{EMOJI_SUCCESS} Tabella signature_chain pulita con successo.{Colors.ENDC}")
...
if __name__ == "__main__":
    # Variabili di connessione al DB, configurabili tramite ENV o con valori di default
    ...
    try:
        ...
        for i, firmatario_info in enumerate(firmatari_list):
            insert_signature_chain(doc_bytes, firmatario_info["nome"], conn_app, firmatario_info["priv"], i == 0, doc_content)
        ...
        if len(firmatari_list) > 1:
            ...
            with conn_app.cursor() as cursor_select_app:
                cursor_select_app.execute("SELECT id FROM signature_chain WHERE signer = %s ORDER BY id ASC LIMIT 1", (target_signer_app,))
            ...
            if tampered_block_data_app:
                ...
                try:
                    with conn_app.cursor() as cursor_update_app:
                        cursor_update_app.execute("UPDATE signature_chain SET document_hash = %s WHERE id = %s", (new_fake_hash_app, tampered_block_id_app))
                    conn_app.commit()
                    print(f"{Colors.FAIL}{EMOJI_FAIL} ERRORE INASPETTATO: L'UPDATE come {app_db_user} è riuscito. Controllare i permessi RLS/GRANT.{Colors.ENDC}")
                except psycopg2.Error as db_error:
                    print(f"{Colors.OKGREEN}{EMOJI_SUCCESS} SUCCESSO: Tentativo di UPDATE BLOCCATO dal DB per '{app_db_user}' come previsto!{Colors.ENDC}")
                    print(f"  {Colors.OKCYAN}Errore DB (SQLSTATE {db_error.pgcode}): {db_error.pgerror.strip()}{Colors.ENDC}")
                    conn_app.rollback() 
            else:
                print(f"{Colors.WARNING}{EMOJI_WARN} Blocco da manomettere (firmatario: {target_signer_app}) non trovato.{Colors.ENDC}")
        else:
            print(f"{Colors.WARNING}{EMOJI_WARN} Non ci sono abbastanza firmatari per testare la manomissione.{Colors.ENDC}")

        verify_chain(conn_app, firmatari_pub_keys, f"{app_db_user} - Post Tentativo UPDATE Bloccato")

    except psycopg2.OperationalError as e:
        print(f"{Colors.FAIL}{EMOJI_FAIL} Errore di connessione ({app_db_user}): {e}{Colors.ENDC}")
        ...
        print(f"{Colors.FAIL}{EMOJI_FAIL} Errore imprevisto nello scenario {app_db_user}: {error}{Colors.ENDC}")
        if conn_app: conn_app.rollback()
    finally:
        if conn_app:
            conn_app.close()
    ...
    try:
        ...
        print(f"{EMOJI_DB} Tentativo di connessione al database '{db_name}' come utente '{Colors.WARNING}{super_db_user}{Colors.ENDC}'...")
        conn_super_scenario = psycopg2.connect(dbname=db_name, user=super_db_user, password=super_db_password, host=db_host)
        print(f"{Colors.OKGREEN}{EMOJI_SUCCESS} Connessione come '{super_db_user}' riuscita.{Colors.ENDC}")

        for i, firmatario_info in enumerate(firmatari_list):
            insert_signature_chain(doc_bytes, firmatario_info["nome"], conn_super_scenario, firmatario_info["priv"], i == 0, doc_content)
        ...
        if len(firmatari_list) > 1:
            ...
            with conn_super_scenario.cursor() as cursor_select_super:
                cursor_select_super.execute("SELECT id FROM signature_chain WHERE signer = %s ORDER BY id ASC LIMIT 1", (target_signer_super,))
            ...
            if tampered_block_data_super:
                ...
                try:
                    with conn_super_scenario.cursor() as cursor_update_super:
                        cursor_update_super.execute("UPDATE signature_chain SET document_hash = %s WHERE id = %s", (new_fake_hash_super, tampered_block_id_super))
                    conn_super_scenario.commit()
                    print(f"{Colors.OKGREEN}{EMOJI_SUCCESS} Manomissione UPDATE (come {super_db_user}) effettuata con successo a livello DB.{Colors.ENDC}")
                except psycopg2.Error as db_error:
                    print(f"{Colors.FAIL}{EMOJI_FAIL} ERRORE INASPETTATO: L'UPDATE come '{super_db_user}' è fallito: {db_error}{Colors.ENDC}")
                    conn_super_scenario.rollback()
            else:
                print(f"{Colors.WARNING}{EMOJI_WARN} Blocco da manomettere (firmatario: {target_signer_super}) non trovato.{Colors.ENDC}")
        else:
            print(f"{Colors.WARNING}{EMOJI_WARN} Non ci sono abbastanza firmatari per testare la manomissione.{Colors.ENDC}")

        verify_chain(conn_super_scenario, firmatari_pub_keys, f"{super_db_user} - Post Manomissione DB")

    except psycopg2.OperationalError as e:
        print(f"{Colors.FAIL}{EMOJI_FAIL} Errore di connessione ({super_db_user}): {e}{Colors.ENDC}")
    except (Exception, psycopg2.Error) as error:
        print(f"{Colors.FAIL}{EMOJI_FAIL} Errore imprevisto nello scenario {super_db_user}: {error}{Colors.ENDC}")
        if conn_super_scenario: conn_super_scenario.rollback()
    finally:
        if conn_super_scenario:
            conn_super_scenario.close()
            print(f"\n{EMOJI_DB} Connessione '{super_db_user}' chiusa.")

    print(f"\n{Colors.BOLD}{Colors.OKGREEN}Fine della dimostrazione.{Colors.ENDC}")